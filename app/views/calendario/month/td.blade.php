<div 
  ...
  <div class="divEvents" data-numero-de-eventos="{{count($sgrDia->events())}}">
    ...
    @if (count($sgrDia->events()) > 4) <a style="display:none" class="cerrar" href="">Cerrar</a>@endif
      ...
      <div class="divEvent" data-fecha="{{date('j-n-Y',$sgrDia->timestamp())}}" data-hora="{{substr($event->horaInicio,0,2)}}">
        <a class = "linkpopover linkEvento linkpopover_month
            ...
            data-title="{{ sgrDate::parsedatetime($event->horaInicio,'H:i:s','G:i')}}-{{sgrDate::parsedatetime($event->horaFin,'H:i:s','G:i')}}
                        {{ $event->titulo }}" 
            data-content="{{htmlentities( (string) View::make('calendario.allViews.tooltip')->with('sgrDia',$sgrDia)->with('sgrRecurso',$sgrDia->sgrRecurso())->with('time',$sgrDia->timestamp())->with('event',$event) )}}"
            data-toggle="popover"
            data-trigger="focus"    
        >
  ...
  @if (count($sgrDia->events()) > 4) 
    <a class="linkMasEvents" href=""> + {{ (count($sgrDia->events())-4) }}  más </a>
  @endif
