<p style="width=100%;text-align:center" class=" alert @if ($sgrDia->haySolape(strtotime($event->horaInicio),strtotime($event->horaFin))) alert-danger @elseif ($event->estado == 'aprobada') alert-success @elseif ($event->estado == 'pendiente') alert-danger @endif">
		
		Estado:<strong> Solicitud @if ($sgrDia->haySolape(strtotime($event->horaInicio),strtotime($event->horaFin))) Solapada @else {{ucfirst($event->estado)}} @endif</strong>
...
<p style="width=100%;text-align:center">
	{{ucfirst(strftime('%a, %d de %B, ',$time))}}{{ sgrDate::parsedatetime($event->horaInicio,'H:i:s','G:i')}} - {{sgrDate::parsedatetime($event->horaFin,'H:i:s','G:i') }}
</p>
<p style="width=100%;text-align:center">
	{{$event->actividad}}
</p>
...
@endif
@if($event->esFinalizable() && $sgrRecurso->atendidoPor(Auth::user()->id) )
