...
@foreach($grupos as $grupo)
      ...
      <li  class="list-group-item col-md-12 listitem" >
        <a href= "#divgrupo_{{$grupo->id}}" class="text-success toggle listadorecursos" data-divrecursosid="#divrecursos_{{$grupo->id}}"><i class="fa fa-angle-double-down fa-fw"></i>{{$grupo->nombre}}</a>

        <span style="display:none" id="divgrupo_{{$grupo->id}}" class="opcionesGrupo">
              ...
              <ul style="margin-left:1.5em;display:none" id="divrecursos_{{$grupo->id}}" class="list-group"><!-- recurso en el grupo -->
                @foreach($grupo->recursos as $recurso)
                    <li class="list-group-item">
                      ...
                      <a href= "#divitems_{{$recurso->id}}" class="@if($recurso->disabled) text-warning @else text-success @endif toggleOptionsRecurso listadoItems">@if($recurso->items->count() > 0) <i class="fa fa-angle-double-down fa-fw"></i> @endif {{$recurso->nombre}}</a>
