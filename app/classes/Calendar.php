	...
	private function getContentTD($day,$mon,$year,$id_recurso,$events,$hour=0,$min=0,$view='month',$enabled='false'){
        ...
        foreach($events as $event){
        	...
        	if ($event->estado == 'denegada'){
				...
				$alert = '<span data-toggle="tooltip" title="Solicitud denegada" class=" glyphicon glyphicon-ban-circle text-danger" aria-hidden="true"></span>';
        	}
        	else if ($event->estado == 'aprobada'){
				...
				$where .= " or (horaFin < '".$hf."' and horaFin > '".$hi."'))";
				$nSolapamientos = Recurso::find($id_recurso)->events()->whereRaw($where)->count();*/
				//$nSolapamientos = Calendar::hasSolapamientos($event->id,$event->recurso_id);
				...
				if ($haysolape == true){
					...
					$alert = '<span data-toggle="tooltip" title="Solicitud con solapamiento" class="glyphicon glyphicon-exclamation-sign text-warning" aria-hidden="true"></span>';
				}
				else {
        	...
        	if($event->estado == 'denegada') $classEstado = "alert alert-danger";
